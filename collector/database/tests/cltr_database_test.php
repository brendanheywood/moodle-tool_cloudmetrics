...
class cltr_database_test extends \tool_cloudmetrics\metric_testcase {
    ...
    public function test_expiry() {
        ...
        $daysago = time() - (int)(19.5 * lib::SECS_IN_DAY); // 19.5 days ago.
        $stub = $this->get_metric_stub([1, 2, 3], $daysago, lib::SECS_IN_DAY);
        $collector = new collector();
        ...
        set_config('metric_expiry', 10, 'cltr_database');

        $task = new \cltr_database\task\expire_metrics();
        $task->execute();
